...
class Conversation{
     ...
     * */
    static _getSocketConnectionManager(self, server){
     ...
     * */
    static _getPeerConnectionManager(self){
    ...
    _participate(){
        ...
        this.bc.receiveCall()                                                         // when a user sends that he intends to call over broadcast
            .then(function(call) {                                                    // check if it he wants to call you and if it is not your own call
                return new Promise(function(resolve, reject){                         // create a private peer and socket connection between you and the user
            ...
            .then(this.sc.sendAnswer)                                                  // on any error, check if it was a call not for you --> restart or a real error
            .catch(function(err){ err === false ? this._participate() : console.log(err)});
        if(typeof this.onCalled === "function") this.onCalled(pc, sc)               // give the onCalled callback the active socket and peer connection
    }
